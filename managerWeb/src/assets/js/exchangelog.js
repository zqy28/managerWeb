...
window.$ = $;
window.jQuery = $;
$("#navi-exchangelog").on("exchangelogEvent", function () {

    //读取第一页游戏信息
    $(".data-loading").stop(true, true).fadeIn(100);
    $.ajax({
        type: 'POST',
        url: 'getExchangelogAction',
        ...
        success: (data) => {
            ...
            for (let i = 0; i < data.exchangeLogs.length; i++) {
                $("#game-rewardlog-tbody").append(
                    `<tr>
                        <td>${data.exchangeLogs[i].exchangeLogId}</td>
                        <td>${data.exchangeLogs[i].userId}</td>
                        <td>${data.exchangeLogs[i].commodity.name}</td>
                        <td>${data.exchangeLogs[i].credit}</td>
                        <td>${data.exchangeLogs[i].addtime}</td>
                    </tr>`
                );
    ...
    $("#jump-navipage-btn").click(function () {
        ...
        if (isNaN($("#jump-navipage-input").val())||parseInt($("#jump-navipage-input").val())<1||parseInt($("#jump-navipage-input").val()) >parseInt( $(`.normal-list-nav`).attr("pageSum"))) {
            $(".alert-danger").html("请输入有效页数!").stop(true, true).fadeIn(200).delay(2000).fadeOut(200);
        } else {
